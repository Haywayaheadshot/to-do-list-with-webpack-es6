...
export default class Todo {
  ...
  removeTodo(index) {
    const toDos = this.getTodos();
    const data = toDos.filter(this.cleanData(index));
    const update = data.map(this.updateID(index));
  ...
  updateTodo(index, value) {
    const toDos = this.getTodos();
    const update = toDos.map(this.updateData(index, value));
    localStorage.setItem('todos', JSON.stringify(update));
  }
  ...
  markComplete(index, completed) {
    const toDos = this.getTodos();
    const update = toDos.map(this.updateStatus(index, completed));
    localStorage.setItem('todos', JSON.stringify(update));
  }
